...
def get_price_direction(asset_price_history, no_of_days = 10, end_date = np.NaN):
    ...
    if asset_price_history_truncated.loc[-1]['Close']>asset_price_history_truncated.shift(no_of_days)['Close']:
        return 1
    else if asset_price_history_truncated.loc[-1]['Close']==asset_price_history_truncated.shift(no_of_days)['Close']:
        return 0
    else:
        return -1
